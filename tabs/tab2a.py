...
def display_tab2b(tab2, df):
    ...
    coupon_counts = df.groupby(['year_quarter', 'coupon_status']).size().reset_index(name='count')
    coupon_counts['year_quarter'] = coupon_counts['year_quarter'].astype(str)

    # Calculate the total transactions per year_quarter
    total_transactions = coupon_counts.groupby('year_quarter')['count'].sum().reset_index()
    ...
    fig = px.bar(
        ...
        color_discrete_map={'Used': 'lightgreen', 'Not Used': 'lightcoral'},  # Specify colors
    )
...
def display_tab2c(tab2, df):
    ...
    revenue_2018 = df[df['date'].dt.year == 2018].groupby('quarter')['total_spent'].sum().reset_index()
    revenue_2019 = df[df['date'].dt.year == 2019].groupby('quarter')['total_spent'].sum().reset_index()
