    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            for idx, row in products_df.iterrows():
                image_path = os.path.join(current_dir, 'images', f'{row["product_id"]}.jpg')
                cur.execute("""
                ...
                """, (row['product_id'], row['product_name'], row['category'], row['generated_title1'], row['generated_description1'], row['generated_title2'], row['generated_description2'], image_path))
                conn.commit()
