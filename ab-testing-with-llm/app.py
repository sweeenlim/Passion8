...
def get_products():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT product_id, product_name, image_path FROM generated_products")
            return cur.fetchall()
...
def select_variant(variant, product_id):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(f"SELECT {title}, {description}, image_path FROM generated_products WHERE product_id = %s", (product_id,))
            return cur.fetchone()
